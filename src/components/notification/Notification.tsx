...
export default function Notification({ type, message }: NotificationProps) {
  ...
  if(type === NotificationTypesEnum.DANGER) {
    return (
      <div className={`fixed bottom-12 md:bottom-4 right-4 transform transition-transform duration-500 
  ...
  if(type === NotificationTypesEnum.DARK) {
    return (
      <div className={`fixed bottom-12 md:bottom-4 right-4 transform transition-transform duration-500 
        ${visible ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'}`}>
  ...
  if(type === NotificationTypesEnum.INFO) {
    return (
      <div className={`fixed bottom-12 md:bottom-4 right-4 transform transition-transform duration-500 
        ${visible ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'}`}>
  ...
  if(type === NotificationTypesEnum.SUCCESS) {
    return (
      <div className={`fixed bottom-12 md:bottom-4 right-4 transform transition-transform duration-500 
        ${visible ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'}`}>
  ...
  if(type === NotificationTypesEnum.WARNING) {
    return (
      <div className={`fixed bottom-12 md:bottom-4 right-4 transform transition-transform duration-500 
        ${visible ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'}`}>
